...
baseProtocol.prototype = {
  ...
  login                : "",
  password             : "",
  keepAliveMode        : false,          // keep the connection alive with NOOP's
  networkTimeout       : 30,             // how many seconds b/f we consider the connection to be stale and dead
  proxyHost            : "",
  proxyPort            : 0,
  proxyType            : "",
  reconnectAttempts    : 0,              // how many times we should try reconnecting
  reconnectInterval    : 10,             // number of seconds in b/w reconnect attempts
  reconnectMode        : false,          // true if we want to attempt reconnecting
  ...
  cleanupEx              : function() { alert('NOT_IMPLEMENTED'); },
  ...
  onConnected : function() {
    ...
    this.isReconnecting    = false;
    this.reconnectsLeft    = 0;                                                  // setup reconnection settings
...
function setProtocol(protocol) {
  var protocolMap = { 'ssh2' : { 'transport': ssh2Mozilla, 'observer': ssh2Observer } }; //ssh2.js & ssh2Observer.js

  gConnection = new protocolMap[protocol].transport(new protocolMap[protocol].observer());
  gConnection.errorConnectStr = bbsfox.getLMBundle().GetStringFromName("errorConn");
