...
ssh2Mozilla.prototype = {
  ...
  connect : function(reconnect) {
    ...
    try {                                                                        // create a control socket
      ...
      var dataListener = {                                                       // async data listener for the control socket
        onStartRequest  : function(request, context) { gCli.onConnect(); },
            ...
            onSocketAccepted : function(serv, transport) {
              ...
              var channelData = function() {
                try {
                  ...
                  self.setTimer(false, channelData, self.refreshRate);
                  return;
                }
                if (data) {
              ...
              self.setTimer(false, channelData.bind(self), 0); // get data from remote host & send to local
  ...
  cleanupEx : function() {
    ...
    delete this.controlInstream;
    delete this.controlInstream;
    delete this.controlOutstream;
    delete this.controlTransport;
    this.controlInstream = this.controlInstream = this.controlOutstream = this.controlTransport = null;
  },
  ...
  check_stderr : function() {
    try {
      ...
      this.setTimer(false, this.input.bind(this), this.refreshRate);
      return;
    }
    if (stderr) {
  ...
  send_output : function(key) {
    while (this.channels[key]['bufferOut'].length > 0) {
      try {
        ...
        this.setTimer(false, wait_callback, this.refreshRate);
        return;
      }
      if (n <= 0) { // eof
        break;
